...
std::string displayMenu() {
    std::string message = "1) List countries with available data\n2) Change current country loaded into system\n"
        "3) Display country's Covid-19 statistics for a certain data\n4) Sort countries by current highest case count\n"
...
//recieves a given vector of countries and sort it by the country's death count
void SelectionSortDeaths(std::vector<Country>& countryVec) {

    for (int i = 0; i < countryVec.size() - 1; i++) {
        int min_index = i;
        for (int j = i + 1; j < countryVec.size(); j++) {
            if (countryVec[j].getCumalativeDeaths("10/28/2020") < countryVec[min_index].getCumalativeDeaths("10/28/2020"))
                min_index = j;
        }
...
void SelectionSortCases(std::vector<Country>& countryVec) {
    ...
    for (int i = 0; i < countryVec.size() - 1; i++) {
        int min_index = i;
        for (int j = i + 1; j < countryVec.size(); j++) {
            if (countryVec.at(j).getCumalativeCases("10/28/2020") < countryVec.at(min_index).getCumalativeCases("10/28/2020"))
                min_index = j;
        }

        //make the sorting swap here
        Country temp = countryVec.at(min_index);
        countryVec.at(min_index) = countryVec.at(i);
        countryVec.at(i) = temp;
    }
}
...
void printVectorDeaths(std::vector<Country>& countryVec, bool ascending) {
    ...
    if (ascending) {
        ...
        for (int i = 0; i < countryVec.size(); i++) {
            std::cout << i + 1 << ": " << countryVec[i].getName() << ": " << countryVec[i].getCumalativeDeaths("10/28/2020") << " deaths" << std::endl;
        }
    }
    else {
        int count = 1;
        for (int i = countryVec.size() - 1; i >= 0; i--) {
            std::cout << count << ": " << countryVec[i].getName() << ": " << countryVec[i].getCumalativeDeaths("10/28/2020") << " deaths" << std::endl;
            count++;
        }
    }
}
...
void printVectorCases(std::vector<Country>& countryVec, bool ascending) {
    ...
    if (ascending) {
        for (int i = 0; i < countryVec.size(); i++) {
            std::cout << i + 1 << ": " << countryVec[i].getName() << ": " << countryVec[i].getCumalativeCases("10/28/2020") << " cases" << std::endl;
        }
    }
    else {
        int count = 1;
        for (int i = countryVec.size() - 1; i >= 0; i--) {
            std::cout << count << ": " << countryVec[i].getName() << ": " << countryVec[i].getCumalativeCases("10/28/2020") << " cases" << std::endl;
            count++;
        }
    }
}
...
int main(int argc, const char* argv[]) {
    ...
    std::ifstream file("Project3Data.csv");
    //Make the unordered_map
    std::unordered_map<std::string, Country> dataMap;
    //helper variables
    std::string line;
    std::string date;
    ...
    int newCases = 0;
    int cumalativeCase = 0;
    int newDeaths = 0;
    int cumalativeDeaths = 0;
    ...
    std::cout << "\t\t\t\t\t\tWelcome to Disease Data Displayer\n\t\t\t\t\t\t---------------------------------\nUsing this program you will be able to compare and contrast COVID statistics across coutries\n\t\t\t\t\t\t\t\tLet's get started!\n\n";
    ...
    for (int i = 0; i < numberOfCountries; i++) {
        ...
        while (std::getline(file, line) && dateCounter < numberOfDates) {
            ...
            //Get cumulativeCase
            tempString = line.substr(0, line.find(','));
            cumalativeCase = stoi(tempString);
            line = line.substr(tempString.size() + 1, line.size());
            //Get newDeaths
            tempString = line.substr(0, line.find(','));
            ...
            //Get cumulative
            tempString = line.substr(0, line.find(','));
            cumalativeDeaths = stoi(tempString);
            //insert data into country
            country.insertDateData(date, newCases, cumalativeCase, newDeaths, cumalativeDeaths);
    ...
    //Display that countries were succussfully loaded into system
    ...
    while (option != 8) {
        ...
        case 3:
            ...
            std::cout << "Cumulative Cases: " << currentCountry.getCumalativeCases(dateString) << std::endl;
            std::cout << "New Deaths: " << currentCountry.getNewDeaths(dateString) << std::endl;
            std::cout << "Cumulative Deaths: " << currentCountry.getCumalativeDeaths(dateString) << std::endl;
