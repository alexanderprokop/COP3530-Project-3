...
Node* AVL::Insert(Node* node, Country country) {
	...
	if (country.getName() < node->key.getName()) {
		...
		node->key.insertDateData(country.getDate(), country.getNewCases(currentDate), country.getCumalativeCases(currentDate), country.getNewDeaths(currentDate), country.getCumalativeDeaths(currentDate));
		root = node;
		return node;
	}
	//increase heights;
